...
def create_train_validation_loaders(dataset: Dataset, validation_ratio,
    ...
    test_smp = sampler.SubsetRandomSampler(test)
    dl_train = torch.utils.data.DataLoader(dataset, batch_size, shuffle=False, sampler=train_smp, num_workers=num_workers)
    dl_valid = torch.utils.data.DataLoader(dataset, batch_size, shuffle=False, sampler=test_smp, num_workers=num_workers)
    # ========================
